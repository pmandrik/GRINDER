...
class HHWWggCustomize():
    ...
    def customizeTagSequence(self):
        ...
        self.process.load("Analysis.GRINDER.flashggHHWWggTag_cff")

        if self.customize.doHHWWggTagCutFlow:
            self.process.GrinderflashggHHWWggTag.doHHWWggTagCutFlowAnalysis = cms.bool(True)
        ...
        self.process.flashggTagSequence.replace(self.process.flashggTagSorter,self.process.grinder_flashggHHWWggTagSequence)

        #self.process.flashggTagSequence.replace(self.process.flashggTagSorter,self.process.grinder_flashggHHWWggTagSequence*self.process.flashggTagSorter)
        #self.process.flashggTagSorter.TagPriorityRanges = cms.VPSet( cms.PSet(TagName = cms.InputTag('GrinderflashggHHWWggTag')) )
 
    def HHWWggTagMerger(self,systlabels=[]):
        self.process.p.remove(self.process.flashggTagSorter)
        self.process.p.replace(self.process.flashggSystTagMerger,self.process.grinder_flashggHHWWggTagSequence*self.process.flashggTagSorter*self.process.flashggSystTagMerger)
