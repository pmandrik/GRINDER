...
#define DEBUG_Grinder_miniAOD 1
...
class Grinder_miniAOD : public edm::one::EDAnalyzer<edm::one::WatchRuns, edm::one::SharedResources>  {
   public:
      explicit Grinder_miniAOD(const edm::ParameterSet&);
      ~Grinder_miniAOD();
...
//
Grinder_miniAOD::Grinder_miniAOD(const edm::ParameterSet& iConfig) // : 
...
Grinder_miniAOD::~Grinder_miniAOD(){}
...
using namespace edm;
void Grinder_miniAOD::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  
  

  /*
  #ifdef DEBUG_Grinder_miniAOD
    std::cout << "Grinder_miniAOD::analyze() ... " << std::endl;
  #endif
  ...
  if( triggerResults->parameterSetID() != prevTriggerParameterSetID ){
    ...
    for(int i = 0, N_triggers = selections_triggers_names.size(); i < N_triggers; i++){
      ...
      if( trigger_indexes[i] < 0 ) std::cout << "Grinder_miniAOD::WARNING, Triger " << trigger_name << " not in trigger menu" << std::endl;
    }
  }
...
void Grinder_miniAOD::beginRun(edm::Run const & run, edm::EventSetup const & setup){
  #ifdef DEBUG_Grinder_miniAOD
    std::cout << "Grinder_miniAOD::beginRun ... " << std::endl;
  #endif
/*
...
void Grinder_miniAOD::endRun(edm::Run const& run, edm::EventSetup const& setup){};

// ------------ method called once each job just before starting event loop  ------------
void Grinder_miniAOD::beginJob(){
}

// ------------ method called once each job just after ending the event loop  ------------
void Grinder_miniAOD::endJob(){
  // outTreeMeta->Fill();
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void Grinder_miniAOD::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(Grinder_miniAOD);
